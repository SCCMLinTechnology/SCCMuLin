...
- (id)initWithTarget:(id)target action:(SEL)action {
    ...
    if (self) {
        ...
        _allowHorizontalScrolling = NO;
        _allowVerticalScrolling = NO;
        _minimumPressDuration = 0.5;
        _minimumMovement = 0;
        _maximumMovement = 10;
        _autoScrollInsets = UIEdgeInsetsMake(0,0, 0,0);
        _frame = CGRectNull;
    }
    return self;
}
...
- (CGPoint)translationInView:(UIView *)view {
    //+_amountScrolled.x , + _amountScrolled.y
    CGPoint totalTranslationInWindow = CGPointMake(_translationInWindow.x  ,
                                                   _translationInWindow.y );
